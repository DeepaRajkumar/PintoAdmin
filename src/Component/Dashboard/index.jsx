...
function Dashboard() {
  ...
  const orderData = [
    { shopName: "KFC", status: "OPEN", orders: "75", live: "05", completed: "70" },
    { shopName: "Burger King", status: "OPEN", orders: "75", live: "05", completed: "70" },
    { shopName: "Thalappakatti Biryani", status: "OPEN", orders: "75", live: "05", completed: "70" },
    { shopName: "Ponram Biryani", status: "OPEN", orders: "75", live: "05", completed: "70" },
    { shopName: "Aachi's Restaurant", status: "10:00 PM", orders: "75", live: "-", completed: "75" },
    { shopName: "Velu Biryani", status: "OPEN", orders: "75", live: "05", completed: "70" }
  ];

  const deliveryPartners = [
    { partnerId: "#001258", status: "ONLINE", orders: "75", live: "05", completed: "70" },
    { partnerId: "#001258", status: "OFFLINE", orders: "75", live: "05", completed: "70" },
    { partnerId: "#001258", status: "ONLINE", orders: "75", live: "05", completed: "70" },
    { partnerId: "#001258", status: "LEAVE", orders: "-", live: "-", completed: "-" },
    { partnerId: "#001258", status: "ONLINE", orders: "75", live: "05", completed: "70" },
    { partnerId: "#001258", status: "ONLINE", orders: "75", live: "05", completed: "70" }
  ];
        ...
        <div className="p-6 space-y-8">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 ">
            {summaryMetrics.map((section, idx) => ( 
              ...
              <h2 className={`absolute ${section.title=="Delivery partner" ? "left-[7.2rem]" : "left-[7.9rem]" }  -top-5 font-medium ${section.color}`}>{section.title}</h2>
                      ...
                      <tr key={index} className="border-t">
                        ...
                        <td>
                          <span className={`px-2 py-1 rounded text-xs
                            ${row.status === 'OPEN' ? 'text-green-600' : 
                              row.status.includes('PM') ? 'text-red-600' : 'text-gray-600'}`}>
                            {row.status}
                          </span>
                        </td>
                        <td>{row.orders}</td>
                  ...
                  <tbody>
                    {deliveryPartners.map((partner, index) => (
                      <tr key={index} className="border-t">
                        ...
                        <td>
                          <span className={`px-2 py-1 rounded text-xs
                            ${partner.status === 'ONLINE' ? 'text-green-600' :
                              partner.status === 'OFFLINE' ? 'text-red-600' :
                              partner.status === 'LEAVE' ? 'text-orange-600' : 'text-gray-600'}`}>
                            {partner.status}
                          </span>
                        </td>
